...
for (let index = 0; index < array.length; index++) {
    const e = array[index];
     ...
     if(e.href.includes("/songs")){
        ...
        let a= fetch(`http://127.0.0.1:3000/songs/${folder}/info.json`)

         let response=await (await a).json();
        //  console.log(response)

         cardContainer.innerHTML=cardContainer.innerHTML+`<div data-folder="${folder}" class="card" id="1">
         <div  class="playButton">
             <?xml version="1.0" ?>
             <!DOCTYPE svg
                 PUBLIC '-//W3C//DTD SVG 1.1//EN' 'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd'><svg
                 height="28px" id="Layer_1" version="1.1" viewBox="0 0 512 512" width="28px"
...
document.querySelector(".range").getElementsByTagName("input")[0].addEventListener("change",(e)=>{
    ...
    currentSong.volume =(e.target.value)/100 // if it is not working use parseInt(e.target.value)/100
